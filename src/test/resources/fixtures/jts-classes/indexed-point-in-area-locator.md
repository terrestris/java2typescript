...
```json
{
  "customImports": [
    {
      "packageName": "org.locationtech.jts.geom.util",
      "javaName": "LinearComponentExtracter"
    }
  ]
}
```
...
```
```typescript
import { LinearComponentExtracter } from "../../geom/util/LinearComponentExtracter.ts";
...
export class IntervalIndexedGeometry {
    ...
    private init(geom: Geometry): void {
        let lines: any[] = LinearComponentExtracter.getLines(geom);
    ...
    public query(min: number, max: number): any[];
